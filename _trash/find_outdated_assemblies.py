...
with open(outfpath, 'wt') as outfile:
    ...
    # Write header
    outfile.write('init_ass_id\tinit_accession\tupdated_accession\n')
    ...
    for i, ass_id in enumerate(ass_ids):
        ...
        updated_accession = record['DocumentSummarySet']['DocumentSummary'][0]['LatestAccession']
        ...
        if updated_accession == '':
            updated_accession = 'NA'
        # end if

        outfile.write(f'{ass_id}\t{init_ass_accession}\t{updated_accession}\n')
