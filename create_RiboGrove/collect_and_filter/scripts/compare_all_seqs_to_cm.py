...
# 2. `--ribotyper-fail-seqIDs` -- a file of seqIDs which didn't pass NNN filter, one per line.
...
for fpath in (fasta_seqs_fpath, ribotyper_fail_fpath, rfam_fpath, cmscan_fpath, cmpress_fpath):
...
input_seq_records = rgIO.read_and_filter_fasta(
    fasta_seqs_fpath,
